...
const {generalSettings, paginationSettings} = require('./baseSetting');

const defaultSize = paginationSettings['enable'] ? paginationSettings['default'] : 0;
const sortedSizes = paginationSettings['enable']
  ? [...new Set(paginationSettings['pagination'])].sort((a, b) => a - b)
  : 0;
...
const popParentFolder = (items) => {
  if (items.length > 0 && generalSettings['showParentFolder']) {
...
function setInputFilter(textbox, inputFilter) {
  ['input', 'keydown', 'keyup', 'mousedown', 'mouseup', 'select', 'contextmenu', 'drop'].forEach(function (event) {
    textbox.addEventListener(event, function (_) {
      if (this.value === '') {
...
const initPagSelector = () => {
  if (!paginationSettings['hidden']) {
    dom(selectorTpl).appTo('#sidebar');
    document.querySelector('#pag_select').addEventListener('change', onSelect);
...
const getCachedPref = () => {
  return pag_rows_pref === undefined ? defaultSize : pag_rows_pref;
};
...
const setPref = (size) => {
  const checked = includes(sortedSizes, size) ? size : defaultSize;
  store.put(storekey, checked);
  pag_rows_pref = checked;
};

const init = () => {
  const stored = store.get(storekey);
  setPref(stored === undefined ? defaultSize : stored);
