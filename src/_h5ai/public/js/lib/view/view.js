...
const {generalSettings} = require('./baseSetting');

const sortedSizes = generalSettings['itemSize'].sort((a, b) => a - b);

const checkedModes = intersection(generalSettings['viewMode'], ['details', 'grid', 'icons']);
...
const addCssStyles = () => {
  ...
  const size = generalSettings['maxIconSize'];
  styles.push(`#view .icon img {max-width: ${size}px; max-height: ${size}px;}`);
...
const set = (mode, size) => {
  ...
  size = includes(sortedSizes, size) ? size : sortedSizes[0];
  mode = includes(checkedModes, mode) ? mode : checkedModes[0];
...
const createHtml = item => {
  ...
  if (item.isCurrentParentFolder()) {
    ...
    if (!generalSettings['showParentFolderName']) {
...
const filterPayload = item => {
  ...
  if (item.parent && generalSettings['showParentFolder']) {
...
const init = () => {
  ...
  format.setDefaultMetric(generalSettings['use1024']);
