...
}, allsettings['module']['tree']);

const itemTpl =
  `<div class="item folder">
      <span class="indicator">
          <img src="${resource.image('tree-indicator')}" alt="IMG"/>
      </span>
      <a>
          <span class="icon"><img src="${resource.icon('folder')}" alt="IMG"/></span>
...
const cmpItems = (item1, item2) => {
  ...
  if (settings['sort']['ignoreCase']) {
    val1 = val1.toLowerCase();
    val2 = val2.toLowerCase();
  }
  return settings['sort']['naturalOrder'] ? naturalCmp(val1, val2) : cmp(val1, val2);
};

const update = item => {
  const subfolders = item.getSubfolders();
  const subLen = subfolders.length;
  const subMax = settings['maxFolder'];
...
const init = () => {
  ...
  if (typeof store.get(storekey) !== 'boolean') {
    store.put(storekey, settings.default);
  }
