...
const sortItems = (column, reverse) => {
  ...
  const fn = cmpFn(columnProps[column], reverse, settings['ignoreCase'], settings['naturalOrder']);
...
const getSortFunc = () => {
  let {column, reverse} = getSortOrder();
  return cmpFn(columnProps[column], reverse, settings['ignoreCase'], settings['naturalOrder']);
};
