...
const settings = Object.assign({
  ...
  browser: true,
  default: 'en'
}, allsettings['browsing']['localization']);
...
const loadLanguage = isoCode => {
  ...
  return server.getRequest('/_h5ai/public/l10n/' + isoCode + '.json').then(response => {
    const temp = Object.assign(defaultTranslations, response, {isoCode: isoCode});
    loaded[isoCode] = temp;
    return temp;
  });
};
  ...
  if (languages[storedIsoCode]) {
    ...
    if (browserLang) {
      if (languages[browserLang]) {
        isoCode = browserLang;
      } else if (browserLang.length > 2 && languages[browserLang.substring(0, 2)]) {
        isoCode = browserLang.substring(0, 2);
      }
    }
...
const init = () => {
  ...
  event.sub('view.changed', () => {
    localize(langs, settings['default'], settings['browser']);
  });
};
